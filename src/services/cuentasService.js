...
export const abrirNuevaCuenta = async (clientaId) => {
  ...
  // Calcular el número de cuenta basado en cuentas NO ANULADAS (activas + cerradas normalmente)
  // Excluir las cuentas anuladas del conteo
  const todasLasCuentas = await cuentasRepo.getByClienta(clientaId);
  const cuentasNoAnuladas = todasLasCuentas.filter(c => !c.anulada);
  const numeroCuenta = cuentasNoAnuladas.length + 1;

  // Crear nueva cuenta (una cuenta por venta)
  const cuenta = {
    id: generateId(),
    clientaId,
    numeroCuenta,
    saldo: 0,
    estado: 'ACTIVA',
    anulada: false, // Por defecto no está anulada
