...
import { obtenerResumenPorCategoria, obtenerMovimientosPorCategoria } from '../../services/reportesService';
...
export default function ProductosVendidosScreen({ navigation }) {
    ...
    const cargarDatos = async () => {
        ...
        try {
            ...
            console.log('📊 Resumen por categoría:', data);
    ...
    const cargarMovimientos = async () => {
        ...
        try {
            ...
            // Log para verificar que los montos vienen redondeados
            console.log('🔍 VERIFICANDO MONTOS REDONDEADOS:');
            data.forEach((mov, idx) => {
                mov.prendas.forEach((prenda, pIdx) => {
                    console.log(`Prenda ${idx}-${pIdx}: ${prenda.descripcion}`);
                    console.log(`  - Pagado: ${prenda.montoPagado}`);
                    console.log(`  - Pendiente: ${prenda.montoPendiente}`);
                    console.log(`  - Total: ${prenda.montoTotal}`);
                });
            });
    ...
    const formatCurrency = (value) => {
        // Redondear a .00 o .50 antes de formatear
        const rounded = Math.round(value * 2) / 2;
        return `S/ ${parseFloat(rounded).toFixed(2)}`;
    };
    ...
    const totalPagado = Object.values(resumen || {}).reduce((sum, cat) => sum + (cat.totalPagado || 0), 0);
    const totalPendiente = Object.values(resumen || {}).reduce((sum, cat) => sum + (cat.totalPendiente || 0), 0);
            ...
            <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.totalCard}>
                    ...
                    <View style={styles.totalStats}>
                        <View style={styles.statItem}>
                            <View style={[styles.statIconContainer, { backgroundColor: 'rgba(76, 175, 80, 0.25)' }]}>
                                <Ionicons name="checkmark-circle-outline" size={18} color="rgba(255,255,255,0.95)" />
                            </View>
                            <View style={styles.statInfo}>
                                <Text style={styles.statValor}>{formatCurrency(totalPagado)}</Text>
                                <Text style={styles.statLabel}>Cobrado</Text>
                            </View>
                        </View>

                        <View style={styles.statItem}>
                            <View style={[styles.statIconContainer, { backgroundColor: 'rgba(255, 152, 0, 0.25)' }]}>
                                <Ionicons name="time-outline" size={18} color="rgba(255,255,255,0.95)" />
                            </View>
                            <View style={styles.statInfo}>
                                <Text style={styles.statValor}>{formatCurrency(totalPendiente)}</Text>
                ...
                {categoriaSeleccionada !== 'TODAS' && movimientos.length > 0 && (
                    <>
                        {/* Productos Pagados */}
                        {(() => {
                            const productosPagados = movimientos.filter(mov =>
                                mov.prendas.some(p => p.estadoPago === 'PAGADO')
                            );
                            const prendasPagadas = productosPagados.flatMap(mov =>
                                mov.prendas.filter(p => p.estadoPago === 'PAGADO').map(p => ({ ...p, mov }))
                            );

                            if (prendasPagadas.length > 0) {
                                return (
                                    <View style={styles.seccionContainer}>
                                        <View style={styles.seccionHeaderConBadge}>
                                            <View style={styles.seccionHeaderConIcono}>
                                                <View style={styles.seccionIconoContainer}>
                                                ...
                                                <View>
                                                    ...
                                                    <Text style={styles.seccionSubtitulo}>
                                                        {prendasPagadas.length} artículos • {formatCurrency(prendasPagadas.reduce((sum, p) => sum + ((p.montoTotal || (p.monto * (p.cantidad || 1)))), 0))}
                                                    </Text>
                                                </View>
                                            </View>
                                            <View style={[styles.estadoBadge, styles.estadoBadgePagado]}>
                                                <Text style={styles.estadoBadgeTexto}>Cobrado</Text>
                                            </View>
                                        </View>

                                        <View style={styles.listaArticulos}>
                                            {prendasPagadas.map((prenda, idx) => (
                                                <View key={idx} style={styles.articuloItem}>
                                                    <View style={[
                                                        styles.articuloIcono,
                                                        { backgroundColor: categorias.find(c => c.id === prenda.categoria)?.color + '15' || '#E1F5FE' }
                                                    ]}>
                                                        <Ionicons
                                                            name={categorias.find(c => c.id === prenda.categoria)?.icono || 'pricetag-outline'}
                                                            size={20}
                                                            color={categorias.find(c => c.id === prenda.categoria)?.color || '#45beffff'}
                                                        />
                                                    </View>
                                                    <View style={styles.articuloInfo}>
                                                        <Text style={styles.articuloDescripcion} numberOfLines={1}>{prenda.descripcion}</Text>
                                                        <View style={styles.articuloMetadata}>
                                                            <Ionicons name="person-outline" size={12} color={colors.textTertiary} />
                                                            <Text style={styles.articuloCliente} numberOfLines={1}>{prenda.mov.clientaNombre}</Text>
                                                            <View style={styles.separator} />
                                                            <Ionicons name="calendar-outline" size={12} color={colors.textTertiary} />
                                                            <Text style={styles.articuloFecha}>{formatDate(prenda.fecha || prenda.mov.fecha)}</Text>
                                                        </View>
                                                    </View>
                                                    <View style={styles.articuloMontoContainer}>
                                                        <Text style={styles.articuloMonto}>{formatCurrency(prenda.montoTotal || (prenda.monto * (prenda.cantidad || 1)))}</Text>
                                                        <View style={[styles.estadoPagoChip, styles.estadoPagoChipPagado]}>
                        ...
                        {(() => {
                            const productosPendientes = movimientos.filter(mov =>
                                mov.prendas.some(p => p.estadoPago !== 'PAGADO')
                            );
                            const prendasPendientes = productosPendientes.flatMap(mov =>
                                mov.prendas.filter(p => p.estadoPago !== 'PAGADO').map(p => ({ ...p, mov }))
                            );

                            if (prendasPendientes.length > 0) {
                                // Calcular totales de pendientes
                                const totalPendiente = prendasPendientes.reduce((sum, p) => sum + (p.montoPendiente || (p.montoTotal || (p.monto * (p.cantidad || 1)))), 0);
                                const totalPagadoParcial = prendasPendientes.reduce((sum, p) => sum + (p.montoPagado || 0), 0);

                                return (
                                    <View style={styles.seccionContainer}>
                                        <View style={styles.seccionHeaderConBadge}>
                                            <View style={styles.seccionHeaderInfo}>
                                                <View style={styles.seccionHeaderConIcono}>
                                                    <View style={styles.seccionIconoContainer}>
                                                    ...
                                                    <View>
                                                        ...
                                                        <Text style={styles.seccionSubtitulo}>
                                                            {prendasPendientes.length} artículos
                                                        </Text>
                                                    </View>
                                                </View>
                                                {totalPagadoParcial > 0 && (
                                                    <View style={styles.pagosParciales}>
                                                        <Text style={styles.pagosParcialTexto}>
                                                            Pagado: {formatCurrency(totalPagadoParcial)} • Pendiente: {formatCurrency(totalPendiente)}
                                                        </Text>
                                                    </View>
                                                )}
                                            </View>
                                            <View style={[styles.estadoBadge, styles.estadoBadgePendiente]}>
                                                <Text style={styles.estadoBadgeTexto}>Por cobrar</Text>
                                            </View>
                                        </View>

                                        <View style={styles.listaArticulos}>
                                            {prendasPendientes.map((prenda, idx) => {
                                                const montoTotal = prenda.montoTotal || (prenda.monto * (prenda.cantidad || 1));
                                                const montoPagado = prenda.montoPagado || 0;
                                                const montoPendiente = prenda.montoPendiente || montoTotal;
                                                ...
                                                return (
                                                    <View key={idx} style={styles.articuloItem}>
                                                        ...
                                                        <View style={styles.articuloInfoExtendido}>
                                                            <View style={styles.articuloInfoRow}>
                                                                <Text style={styles.articuloDescripcion} numberOfLines={1}>{prenda.descripcion}</Text>
                                                                <Text style={styles.articuloMontoTotal}>{formatCurrency(montoTotal)}</Text>
                                                            </View>
                                                            <View style={styles.articuloMetadata}>
                                                                <Ionicons name="person-outline" size={12} color={colors.textTertiary} />
                                                                <Text style={styles.articuloCliente} numberOfLines={1}>{prenda.mov.clientaNombre}</Text>
                                                            ...
                                                            {montoPagado > 0 && (
                                                                <View style={styles.progresoContainer}>
                                                                    <View style={styles.progresoInfo}>
                                                                        <View style={styles.progresoTextoContainer}>
                                                                            <Ionicons name="checkmark-circle" size={11} color="#4CAF50" />
                                                                            <Text style={styles.progresoTextoPagado}>
                                                                                Pagado: {formatCurrency(montoPagado)}
                                                                            </Text>
                                                                        </View>
                                                                        <View style={styles.progresoTextoContainer}>
                                                                            <Ionicons name="alert-circle" size={11} color="#FF9800" />
                                                                            <Text style={styles.progresoTextoPendiente}>
                                                                                Falta: {formatCurrency(montoPendiente)}
                                                                            </Text>
                                                                        </View>
                                                                    </View>
                                                            ...
                                                            {/* Si no hay pagos, mostrar solo el monto pendiente */}
                                                            {montoPagado === 0 && (
                                                                <View style={styles.sinPagosContainer}>
                                                                    <Ionicons name="alert-circle-outline" size={12} color="#FF6B6B" />
                                                                    <Text style={styles.sinPagosTexto}>
                                                                        Sin pagos • Debe: {formatCurrency(montoPendiente)}
                                                                    </Text>
                                                                </View>
                                                            )}
                                                        </View>
...
const createStyles = (colors) => StyleSheet.create({
    ...
    seccionHeaderConIcono: {
        flexDirection: 'row',
        alignItems: 'center',
        gap: 10,
    },
    seccionIconoContainer: {
        width: 36,
        height: 36,
        borderRadius: 18,
        backgroundColor: colors.surfaceVariant,
    ...
    estadoPagoChipPagado: {
        backgroundColor: '#E8F5E9',
    },
    estadoPagoChipParcial: {
        backgroundColor: '#FFF3E0',
    },
    estadoPagoChipPendiente: {
        backgroundColor: '#FFEBEE',
    },

    // Nuevos estilos para productos pendientes con progreso
    seccionHeaderInfo: {
        flex: 1,
    },
    pagosParciales: {
